...
class Game:
    def __init__(self, max_turns, max_x, max_y):
        ...
        # self.monster_reached_range = 1040 or (monster hit box: 500 + aggresive speed 540 * 2 turns)
        self.monster_reached_range = 1580
...
def update_current_fish(scans_list: list[Creature], current_scans: list[Creature]):
    for fish in scans_list:
        found = False
        for scan in current_scans:
            if scan.creature_id == fish.creature_id:
                found = True
                break
        if not found:
            current_scans.append(fish)

""" 
* Update removed fish
Check id in radar list and current removed fish list and creature list
if id not in radar list and in creature list, add to removed fish list if not already in
@params: radar_list, removed_fishs
@return: None
"""
def update_removed_fish(radar_list: list[RadarInfos], removed_fishs: list[Fish]):
    for creature in Creatures:
        if isinstance(creature, Creature):  # Ensure creature is an instance of Creature
            if creature.creature_id != -1:
                found = False
                for radar in radar_list:
                    if radar.creature_id == creature.creature_id:
                        found = True
                        break
                if not found:
                    for fish in removed_fishs:
                        if fish.creature_id == creature.creature_id:
                            found = True
                            break
                    if not found:
                        removed_fishs.append(Fish(creature.creature_id, 0, 0, 0, 0, creature.color, creature.type))
...
# @params: fish, player
# @return: bool
def check_fish(creature_id, drone: Drone, player: Player, removed_fishs: list[Fish]) -> bool:
    if any(fish.creature_id == creature_id for fish in player.current_scans) or any(fish.creature_id == creature_id for fish in player.storage_list):
        return False
    if any(fish.creature_id == creature_id for fish in removed_fishs):
        return False
...
################## Functions ##################
def move_to(x, y, light, display):
    light = 1 if light else 0
    print("MOVE {} {} {} {}".format(x, y, light, display))
...
# * Get dodge path
# TODO: calculate itersection point with monster path and change course to avoid it
...
def current_target(drone: Drone, visible_fish: list[Fish], player: Player):
    ...
    for fish in visible_fish:
        ...
        if dist < min_distance and fish.type != -1 and check_fish(fish.creature_id, drone, player, Removed_Fish) == True and fish.creature_x >= GameInfos.fish_escape_x[0] and fish.creature_x <= GameInfos.fish_escape_x[1]:
            min_distance = dist
            closest_fish = fish
    ...
    #radar fish
        ...
        for radar in drone.radar_list:
            if get_type(radar.creature_id, Creatures) != -1 and check_fish(radar.creature_id, drone, player, Removed_Fish) == True:
                ...
                if radar.radar_location == "TL":
                    ...
                    if drone.drone_x <= GameInfos.drone_search_range[1] and drone.drone_y <= GameInfos.drone_search_range[1]:
                        ...
                        continue
        # no fish found
        print("Drone {} No fish found in search function".format(drone.drone_id), file=sys.stderr, flush=True)
...
def current_monster(drone: Drone, visible_fish: list[Fish], player: Player):
    ...
    elif closest_monster != None and distance(drone.drone_x, drone.drone_y, closest_monster.creature_x, closest_monster.creature_y) <= GameInfos.monster_scan_range:
        drone.dodge = False
        drone.light = False
    else:
        drone.dodge = False
...
def NavigationSystem(drone: Drone, visible_fish: list[Fish], player: Player) -> None:
    if drone.emergency == 1:
        ...
        print("Drone {} has target {} {} {}".format(drone.drone_id, drone.fish_target, drone.target.direction if drone.target else None, "({}, {})".format(drone.target.x, drone.target.y) if drone.target else None), file=sys.stderr, flush=True)
        if drone.dodge == True and drone.target is not None:
            print("Drone {} is dodging".format(drone.drone_id), file=sys.stderr, flush=True)
            move_to(drone.target.x, drone.target.y, drone.light, "Dodge")
            drone.target = None
            drone.fish_target = None
            drone.dodge = False
            return
        if drone.target is not None:
            if drone.returning == True:
                if drone.target.y <= drone.drone_y:
                    move_to(drone.target.x, drone.target.y, drone.light, "Returning and Fishing")
                    return
                else:
                    move_to(drone.target.x, GameInfos.drone_scan_return, drone.light, "Returning scans")
                    return
            else:
                move_to(drone.target.x, drone.target.y, drone.light, drone.battery)
                return
        else:
            # Provide default values if drone.target is None
            print("Drone {} has no target".format(drone.drone_id), file=sys.stderr, flush=True)
            move_to(0, 0, drone.light, "No target")
...
for i in range(creature_count):
    ...
    new = Creature(creature_id, color, _type)
    Creatures.append(new)
...
while True:
    ...
    # reset scans per drone (allow to see current scans and forget already saved scans)
    for drone in Willy.drones_list:
        del drone.scans_list[:]
    del Willy.current_scans[:]
    ...
    for i in range(drone_scan_count):
        ...
        for drone in Willy.drones_list:
             if drone_id == drone.drone_id:
                drone.scans_list.append(Creature(creature_id, get_color(creature_id, Creatures), get_type(creature_id, Creatures)))
                break

    # Refill current scans
    for drone in Willy.drones_list:
        update_current_fish(drone.scans_list, Willy.current_scans)
    ...
    # Reset drone radar to track only visible fish
    for drone in Willy.drones_list:
        del drone.radar_list[:]
    ...
    # Add removed fish to list
    update_removed_fish(Willy.drones_list[0].radar_list, Removed_Fish)
    ...
    for fish in Removed_Fish:
        print("Fish {} has been removed".format(fish.creature_id), file=sys.stderr, flush=True)
