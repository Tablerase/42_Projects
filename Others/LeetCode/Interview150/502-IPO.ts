...
// This class was needed since ts doesnt have builtin heap structure
class MaxHeapCustom<T> {
  private heap: T[] = [];
  ...
  private sinkDown() {
    ...
    while (true) {
      ...
      if (rightIdx < length &&
        this.comparator(this.heap[rightIdx], swap === -1 ? element : this.heap[leftIdx]) > 0) {
        swap = rightIdx;
      }
...
function findMaximizedCapital(k: number, w: number, profits: number[], capital: number[]): number {
  ...
  for (let index = 0; index < profits.length; index++) {
    projects.push({ id: index, cost: capital[index], profit: profits[index] })
  }
  projects.sort((a, b) => a.cost - b.cost)
  console.log("Projects:", projects)

  const heapMax = new MaxHeapCustom<Project>((a, b) => a.profit - b.profit);
  // Make project profits
  let pIndexRef = 0;
  for (let indexK = 0; indexK < k; indexK++) {
    // Update heapMax
    while (pIndexRef < projects.length && projects[pIndexRef].cost <= w) {
      heapMax.push(projects[pIndexRef]);
      pIndexRef++
    }

    // Use heapMax project
    if (heapMax.isEmpty()) break;
    const pUsed = heapMax.pop();
    if (pUsed === undefined) break;
    w = w + pUsed.profit;
